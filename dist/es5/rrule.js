...
var parsestring_assign = (undefined && undefined.__assign) || function () {
    parsestring_assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
    ...
    return parsestring_assign.apply(this, arguments);
};
...
function parseString(rfcString) {
    ...
    return parsestring_assign(parsestring_assign({}, options[0]), options[1]);
}
...
function rebuildYear(year, options) {
    ...
    var result = yearinfo_assign(yearinfo_assign({ yearlen: yearlen, nextyearlen: nextyearlen, yearordinal: yearordinal, yearweekday: yearweekday }, baseYearMasks(year)), { wnomask: null });
...
function iter(iterResult, options) {
    ...
    var counterDate = getInitialCounterDate(iterResult, options);
...
function getInitialCounterDate(iterResult, options) {
    var counterDate = DateTime.fromDate(options.dtstart);
    var minDate = iterResult.minDate;
    ...
    switch (options.freq) {
        ...
        case Frequency.HOURLY:
            return jumpToMinuteBasedInterval(options.dtstart, minDate, options.interval * 60 * 60 * 1000);
        case Frequency.MINUTELY:
            return jumpToMinuteBasedInterval(options.dtstart, minDate, options.interval * 60 * 1000);
        case Frequency.SECONDLY:
            return jumpToMinuteBasedInterval(options.dtstart, minDate, options.interval * 1000);
        default:
            return counterDate;
    }
}
function jumpToDailyInterval(counterDate, minDate, interval) {
    var elapsedDays = Math.floor((Date.UTC(minDate.getUTCFullYear(), minDate.getUTCMonth(), minDate.getUTCDate()) -
        Date.UTC(counterDate.year, counterDate.month - 1, counterDate.day)) /
        (24 * 60 * 60 * 1000));
    var intervals = Math.floor(elapsedDays / interval);
    if (intervals > 0) {
        counterDate.addDaily(intervals * interval);
    }
    return counterDate;
}
function jumpToMinuteBasedInterval(dtstart, minDate, intervalMs) {
    var elapsedIntervals = Math.floor((minDate.getTime() - dtstart.getTime()) / intervalMs);
    if (elapsedIntervals <= 0) {
        return DateTime.fromDate(dtstart);
    }
    return DateTime.fromDate(new Date(dtstart.getTime() + elapsedIntervals * intervalMs));
}
...
function groomRruleOptions(val, dtstart, tzid) {
    return rrulestr_assign(rrulestr_assign({}, val), { dtstart: dtstart, tzid: tzid });
}
function rrulestr_initializeOptions(options) {
    ...
    return rrulestr_assign(rrulestr_assign({}, rrulestr_DEFAULT_OPTIONS), options);
}
